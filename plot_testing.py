...
if __name__ == '__main__':
    ...
    path = ''
    modelfname = 'CNN_at25000_itr35001time1638706685.pt'
    modelfname_parts = modelfname.split('_')
    modelfname_short =modelfname_parts[0]+model_fname_parts[1]
    ...
    trained_params = torch.load(fname, map_location=torch.device(device))
    model = CNN()
    model.load_state_dict(trained_params, strict=True)
    model.eval()
    #print(train_dataloader.batches)
    pred_mass_history = []
    true_mass_history = []
    with torch.no_grad():
        ...
        for test_batch, (_x, _y) in enumerate(train_dataloader):
            ...
            if test_batch == 2000:
                np.save(model_fname_short+"pred_masses.npy", pred_mass_history)
                np.save(model_fname_short+"true_masses.npy", true_mass_history) 
                ...
                #plt.savefig("violin" + str(num_iterations) + "time" + str(int(time.time())) + ".pdf")
                #f2, a, m = pp.plot_histogram_predictions(pred_mass_history, true_mass_history, radius_bins=False, particle_ids=None, errorbars=False,
                #                         label="Raw density", color="C0")
                #plt.savefig("residuals" + str(num_iterations) + "time" + str(int(time.time())) + ".pdf")
                #plt.show()
                sys.exit()
