        ...
        for j in range(subbox_length):
            for k in range(subbox_length):
                output_matrix[i, j, k] = input_matrix[
                    (i + i0) % sim_length, (j + j0) % sim_length, (k + k0) % sim_length]
...
if __name__ == '__main__':
    ...
    if load_model:
        ...
        model.load_state_dict(torch.load('CNN_itr25001time1637715789.pt'))
        model.eval()
        # print(model.gamma)
    ...
    for batch, (_den_field, _true_mass) in enumerate(train_dataloader):
        ...
        if batch % 10 == 0:
            ...
            print(
                f"iteration = {batch}   loss = {loss}  test_loss = {test_loss}  train time = {train_time}  test time = {end - start}")
