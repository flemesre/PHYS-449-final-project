...
if __name__ == '__main__':
    ...
    saved_network = "batch_size_8_CNN_itr35001time1638728894.pt" # The trained parameters to be loaded
    sims = [7]
    training_list = [7]
    # test_sim = 2  # which simulation is used for testing
    num_test_batches = 10000 # The number of test batches to use for plotting
    ...
    with torch.no_grad():
        ...
        for test_batch, (_x, _y) in enumerate(train_dataloader):
            ...
            if test_batch % 10 == 0:
                ...
                print(f"batch = {test_batch}   time = {end-start}")
                start = time.time()
